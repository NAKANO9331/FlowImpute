...
class TemporalPositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout, max_len, lookup_index=None):
        ...
        # Define a constant in the model that won't be updated during .step()
...
def make_model(DEVICE, num_layers, encoder_input_size, decoder_input_size, d_model, adj_mx, adj_phase, mask_matrix, nb_head,
    ...
    if aware_temporal_context:  # employ temporal trend-aware attention
        attn_ss = MultiHeadAttentionAwareTemporalContex_q1d_k1d(nb_head, d_model, num_of_mhalf, points_per_mhalf, kernel_size, dropout=dropout)  # encoder's trend-aware attention uses 1D convolution
        ...
        att_tt = MultiHeadAttentionAwareTemporalContex_qc_kc(nb_head, d_model, num_of_mhalf, points_per_mhalf, kernel_size, dropout=dropout)  # decoder's trend-aware attention uses causal convolution
