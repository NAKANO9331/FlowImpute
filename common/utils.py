...
def norm_Adj(W):
    ...
    N = W.shape[0]
    W = W + np.identity(N)  # Add self-connections to the adjacency matrix
...
def compute_val_loss(net, val_loader, criterion, sw, epoch, mask_matrix, _mean, _std):
    ...
    with torch.no_grad():
        ...
        for batch_index, batch_data in enumerate(val_loader):
            ...
            # decode
            decoder_start_inputs = decoder_inputs[:, :, :1, :]  # Only take the first value of input, then use predicted values as input
